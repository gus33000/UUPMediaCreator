...
namespace Microsoft.Dism
{
    public static partial class DismApi
    {
        ...
        internal static partial class NativeMethods
        {
            ...
            /// <returns><para>Returns OK on success.</para>
            /// <para>Returns E_INVALIDARG if any of the paths are not well-formed or if MountPath or ImageFilePath does not exist or is invalid.</para>
            /// <para>Returns a Win32 error code mapped to an HRESULT for other errors.</para></returns>
            /// <remarks><para>After mounting an image, use the DismOpenSession Function to start a servicing session. For more information, see Using the DISM API.</para>
            /// <para>Mounting an image from a WIM or VHD file that is stored on the network is not supported. You must specify a file on the local computer.</para>
            /// <para>To mount an image from a VHD file, you must specify an ImageIndex of 1.</para>
            /// <para>The MountPath must be a file path that already exists on the computer. Images in WIM and VHD files can be mounted to an empty folder on an NTFS formatted drive. You can also mount an image from a VHD file to an unassigned drive letter. You cannot mount an image to the root of the existing drive.</para>
            /// <para>When mounting an image in a WIM file, the image can either be identified by the image index number specified by ImageIndex, or the name of the image specified by ImageName. ImageIdentifier specifies whether to use the ImageIndex or ImageName parameter to identify the image.</para>
            /// <para>
            ...
            public static extern int DismMountImage(string imageFilePath, string mountPath, uint imageIndex, string imageName, DismImageIdentifier imageIdentifier, uint flags, SafeWaitHandle cancelEvent, DismProgressCallback progress, IntPtr userData);
        }
    }
}